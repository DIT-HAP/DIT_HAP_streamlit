...
def main():
    ...
    with st.expander("🔬 Understanding the Plots", expanded=False):
        st.markdown("""
    ...
    with st.expander("⚙️ Data Requirements", expanded=False):
        st.markdown("""
    ...
    with st.expander("🎯 Analysis Tips", expanded=False):
        st.markdown("""
        **Gene Selection:**
        - Start with well-characterized genes (`cdc2`, `wee1`, `rum1`) as controls
        - Compare essential vs non-essential genes for validation
        - Use systematic IDs for precise gene identification
        - Mix genes from different pathways for diverse analysis

        **Interpreting Depletion Patterns:**
        - **Essential genes**: Show strong depletion across all time points
        - **Non-essential genes**: Little or no depletion observed
        - **Conditionally essential**: Depletion only under specific conditions
        - **Gene length effects**: Longer genes may show different patterns

        **Quality Assessment:**
        - Check error bars and confidence intervals for reliability
        - Verify multiple insertions per gene for robust statistics
        - Look for consistent patterns across different datasets
        - Use quality metrics to filter out problematic data
        """)
